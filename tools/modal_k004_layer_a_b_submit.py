"""Modal runner for Kytos k004 Layer A/B full-panel submission."""
...
OUT_DIR = f"{REPO_DIR}/experiments/k004-layer-a-b-validation"
VCC_NAME = "kytos-k004-layer-a-b"
...
app = modal.App("kytos-k004-layer-a-b")
...
def run_and_submit() -> dict:
    """Clone, build k004 Layer A/B, prep, and submit."""
    t_start = time.time()
    ...
    build_cmd = (
        f"cd {REPO_DIR}\n"
        f"mkdir -p {OUT_DIR}\n"
        "python tools/run_k004_layer_a_b.py \\\n"
        f"  --raw-dir {RAW_DIR} \\\n"
        f"  --out-dir {OUT_DIR} \\\n"
        "  --contexts A,B,C"
    )
    run_step("build k004 Layer A/B prediction", build_cmd)
