"""Kytos k013 — per-context delta scaling anchored on lineage ratios.

The k011 sweep bracketed the uniform delta_scale optimum near 1.7
(x1.3 +0.0559 / x1.7 +0.0596 / x2.0 +0.0566). The lineage-score result
suggests that uniform value is a *mixture* of per-context optima.
Measuring ||delta_lineage|| / ||delta_K562|| on 2,055 shared
essential-screen targets:

  RPE1:   median 1.57   (context B weakly RPE1-leaning)
  Jurkat: median 1.01   (context A strongly Jurkat-like, 0.649)
  hESC:   ~0.44         (context C unresolved/hESC-leaning; k012 LOO scalar)

Mean ratio ~1.0, so the mean-preserving variant keeps the champion's
global magnitude budget and redistributes it:

  A -> 1.7 x 1.01 ~ 1.70   B -> 1.7 x 1.57 ~ 2.65   C -> 1.7 x 0.44 ~ 0.75

First evidence-derived context conditioning (vs a uniform knob). Caveats:
ratios measured on essential genes (panel is non-essential); B/C lineage
assignments are weak; Atlas-sourced deltas are already context-appropriate
but get scaled anyway. Sampler = champion config.
"""
...
RUN_ID = "k013-context-scale-validation"
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument(
        "--delta-scale-map",
        type=str,
        default="A=1.7,B=2.65,C=0.75",
        help="per-context delta scale, e.g. 'A=1.7,B=2.65,C=0.75'",
    )
    ...
    scale_map = {}
    for part in args.delta_scale_map.split(","):
        k, v = part.split("=")
        scale_map[k.strip()] = float(v)
    ...
    meta = {
        ...
        "mode": "replogle_prior+neighbor_imputation+heterogeneous_kd+per_context_delta_scale",
        "contexts": contexts,
        ...
        "delta_scale_map": scale_map,
        "delta_scale_evidence": (
            "lineage ||delta|| ratios vs K562 on 2,055 shared essential "
            "targets: RPE1 1.57, Jurkat 1.01, hESC ~0.44 (k012 LOO scalar); "
            "mean-preserving redistribution of champion uniform 1.7"
        ),
        "library_cap": "median",
