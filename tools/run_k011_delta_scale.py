"""Kytos k011 — champion config + explicit delta-magnitude scaling.

The k009/k010 experiments isolated why the k008 sweep won: the leaderboard
rewards applied delta magnitude, not dispersion-at-mean-1. In applied-mean
terms the k008 sweep was still rising linearly at its edge (mean ~1.4 at
kd_std=2.0 -> +0.0511). This run keeps the champion sampler exactly
(clip(N(1, 2.0), 0), mean ~1.40 / std ~1.49) and scales the assembled delta
by --delta-scale, extending the same axis: s=1.3 -> applied mean ~1.8,
s=1.7 -> ~2.4. library_cap="median" is kept as the overflow backstop since
scaled deltas amplify the heavy-eta tail.
"""
...
RUN_ID = "k011-delta-scale-validation"
...
def main(argv: list[str] | None = None) -> int:
    ...
    meta = {
        ...
        "mode": "replogle_prior+neighbor_imputation+heterogeneous_kd+delta_scale",
        "contexts": contexts,
