"""Modal k010 job: k007 priors + mean-corrected heterogeneous KD sampling.

Same pipeline as k008; the sampler is HeterogeneousTransportSampler with
mean_correct=True — eta ~ N(1, kd_std) truncated at 0, divided by
E[N(1, kd_std) | eta>0] so the applied delta stays mean-1.

Run:
  modal run -d tools/modal_k010_mean_corrected_kd.py::build_and_prep --kd-std 2.0
  modal run -d tools/modal_k010_mean_corrected_kd.py::submit_from_volume --kd-std 2.0

Persists the `.vcc` to the `kytos-vcc` Modal Volume under
/kytos-vol/k010-mean-corrected-kd-s<p>/.
"""
...
app = modal.App("kytos-k010-mean-corrected-kd")
...
OUT_DIR = "/root/kytos/experiments/k010-mean-corrected-kd-validation"
...
def build_and_prep(max_targets: int = 0, kd_std: float = 2.0, submit: bool = False) -> dict:
    t_start = time.time()
    ...
    build_cmd = (
        f"cd {REPO_DIR}\n"
        f"mkdir -p {OUT_DIR}\n"
        "python tools/run_k010_mean_corrected_kd.py \\\n"
        f"  --raw-dir {RAW_DIR} \\\n"
    ...
    run_step("build k010 mean-corrected-KD prediction", build_cmd)
    ...
    tag = f"k010-mean-corrected-kd-s{str(kd_std).replace('.', 'p')}"
    persist_cmd = (
...
def submit_from_volume(kd_std: float = 2.0) -> dict:
    ...
    result = subprocess.run(
        [
            ...
            vcc_path,
            "--model-name",
            f"kytos-k010-mc-s{str(kd_std).replace('.', 'p')}",
            "--wait",
        ],
        stdout=None,
        stderr=subprocess.STDOUT,
