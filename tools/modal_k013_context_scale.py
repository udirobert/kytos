"""Modal k013 job: champion config + per-context delta scaling.

Same pipeline as k011; run_k013_context_scale.py applies a per-context
delta_scale anchored on measured lineage ||delta|| ratios:
A=1.7 (Jurkat ~1.0x K562), B=2.65 (RPE1 ~1.57x), C=0.75 (hESC ~0.44x).
First evidence-derived context conditioning.

Run:
  modal run -d tools/modal_k013_context_scale.py::build_and_prep
  modal run -d tools/modal_k013_context_scale.py::submit_from_volume

Persists the `.vcc` to the `kytos-vcc` Modal Volume under
/kytos-vol/k013-context-scale/.
"""
...
app = modal.App("kytos-k013-context-scale")
...
OUT_DIR = "/root/kytos/experiments/k013-context-scale-validation"
TAG = "k013-context-scale"
...
def build_and_prep(
    max_targets: int = 0,
    scale_map: str = "A=1.7,B=2.65,C=0.75",
    submit: bool = False,
) -> dict:
    t_start = time.time()
    ...
    build_cmd = (
        f"cd {REPO_DIR}\n"
        f"mkdir -p {OUT_DIR}\n"
        "python tools/run_k013_context_scale.py \\\n"
        f"  --raw-dir {RAW_DIR} \\\n"
        ...
        "  --contexts A,B,C \\\n"
        "  --kd-std 2.0 \\\n"
        f"  --delta-scale-map {scale_map} \\\n"
        f"  --max-targets {max_targets}"
    )
    run_step("build k013 context-scale prediction", build_cmd)
