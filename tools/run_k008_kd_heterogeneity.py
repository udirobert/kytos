"""Kytos k008 — k007 priors + heterogeneous per-cell knockdown sampling.

Same prior stack as k007 (Atlas/Replogle real > neighbor-imputed > fallback)
but the Layer B sampler models per-cell knockdown-strength heterogeneity:
each transported cell scales its delta by eta ~ N(1, kd_std), truncated at 0.

Motivation (measured, tools/modal_k008_measure_kd_spread.py): in the 2025
Atlas, the projection of real perturbed cells onto their target's delta has
median per-target std ~1.1 in delta units (~0.2-0.9 for strong signatures);
the k006/k007 additive transport has eta == 1 exactly. This missing spread
along the delta direction is the leading hypothesis for the weak fid
component (-0.359 on k006).
"""
...
RUN_ID = "k008-kd-heterogeneity-validation"
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--kd-std", type=float, default=0.4)
    ...
    meta = {
        ...
        "mode": "replogle_prior+neighbor_imputation+heterogeneous_kd",
        "contexts": contexts,
        ...
        "kd_spread_evidence": "/kytos-vol/k008-kd-spread.json (Atlas eta measurement)",
        "dispatch": totals,
