"""Modal k008 job: k007 priors + heterogeneous per-cell knockdown sampling.

Same pipeline as k007; the sampler scales each cell's delta by
eta ~ N(1, kd_std) truncated at 0 — measured Atlas spread motivates it
(see tools/modal_k008_measure_kd_spread.py, /kytos-vol/k008-kd-spread.json).

Run:
  modal run -d tools/modal_k008_kd_heterogeneity.py::build_and_prep
  modal run -d tools/modal_k008_kd_heterogeneity.py::build_and_prep --kd-std 0.5

Persists the `.vcc` to the `kytos-vcc` Modal Volume under
/kytos-vol/k008-kd-heterogeneity/.
"""
...
app = modal.App("kytos-k008-kd-heterogeneity")
...
OUT_DIR = "/root/kytos/experiments/k008-kd-heterogeneity-validation"
...
def build_and_prep(max_targets: int = 0, kd_std: float = 0.4, submit: bool = False) -> dict:
    t_start = time.time()
    ...
    build_cmd = (
        f"cd {REPO_DIR}\n"
        f"mkdir -p {OUT_DIR}\n"
        "python tools/run_k008_kd_heterogeneity.py \\\n"
        f"  --raw-dir {RAW_DIR} \\\n"
    ...
    run_step("build k008 heterogeneous-KD prediction", build_cmd)
    ...
    tag = f"k008-kd-heterogeneity-s{str(kd_std).replace('.', 'p')}"
    persist_cmd = (
...
def submit_from_volume(kd_std: float = 0.4) -> dict:
